...
export default function OperationTasksPage() {
  ...
  const columns = useMemo<ProColumns<OperationTaskSummary>[]>(() => [
    ...
    { title: '任务类型', dataIndex: 'taskType', width: 130, render: (_, row) => taskTypeLabel(row.taskType) },
    { title: '平台', dataIndex: 'platform', width: 110, render: (_, row) => platformLabel(row.platform) },
    { title: '状态', dataIndex: 'status', width: 130, render: (_, row) => <OperationTaskStatusTag status={row.status} /> },
    { title: '优先级', dataIndex: 'priority', width: 100, render: (_, row) => <OperationPriorityTag priority={row.priority} /> },
    { title: '最新草稿', dataIndex: 'latestDraftVersion', width: 110, render: (_, row) => (row.latestDraftVersion ? `v${row.latestDraftVersion}` : '—') },
    { title: '最新执行状态', dataIndex: 'latestExecutionStatus', width: 130, render: (_, row) => (row.latestExecutionStatus ? <OperationAttemptStatusTag status={row.latestExecutionStatus} /> : '—') },
    { title: '创建人', dataIndex: 'createdBy', width: 130, render: (_, row) => copyableText(row.createdBy || '', 10) },
    { title: '创建时间', dataIndex: 'createdAt', width: 130, render: (_, row) => <DateTimeText value={row.createdAt || ''} /> },
    { title: '更新时间', dataIndex: 'updatedAt', width: 130, render: (_, row) => <DateTimeText value={row.updatedAt || ''} /> },
    {
      title: '操作',
      valueType: 'option',
      width: 120,
