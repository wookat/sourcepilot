...
- The Open API entry (`GET /api/open/v1/*`, R152) reuses the same read-only token system: tokens carry a `purpose` (`mcp` default / `openapi` / `both`) selecting which entry they may call, so a token issued for one surface cannot be replayed against the other. It is equally read-only (GET only), tenant-scoped, audited per call, and rate limited (`OPENAPI_RATE_RPS` / `OPENAPI_RATE_BURST`, buckets independent from MCP). Responses mask customer names and never include internal UUIDs, contact details, raw platform payloads, or credentials. Disable with `OPENAPI_ENABLED=false` if unused.
