...
- **限额计数口径（R188 登记）**：限额刻意等同于审计轨迹（成功 execute 审计行即配额消耗），不设"合成数据"排除标记——任何可由写入方自行声明的排除标记都会破坏 fail-closed 语义（伪造标记即可绕过限额）。因此演示 / 压测等合成审计数据**必须**把 `created_at` 写到配额窗口之外（token 窗口 1 小时、租户窗口 24 小时之前），不得落在当前窗口内；仓库内置 seed 工具不写 MCP 审计行，该约定只约束手工造数脚本。
...
1. **租户金额上限必须先配置**：设置项 `mcp` 组 `mark_paid_single_limit`（单笔上限）与 `mark_paid_daily_limit`（日累计上限），仅管理员可改。缺失、非数字、`0` 或负数一律视为**未配置 = 工具不可用**（默认关）。两个上限值在 `PUT /api/v1/settings` 写入时即做服务端值域校验（R189）：仅接受大于 0、至多两位小数、不超过 1e10（100 亿，与单笔金额硬上界一致）的十进制数字，科学计数法 / 正负号 / 千分位 / NaN / Infinity / 非数字一律 400 拒绝且不落库；空值合法（= 未配置，消费侧 fail-closed）；这两个 key 不允许加密存储（加密行会被消费侧视为未配置）。
