...
R165 修复了六处 view-only 后端越权（审单决定、异常标记族、店铺删除、店铺授权/OAuth 写、店铺同步/重试、刊登目标店），后端统一收口为：view-only 店铺写操作 → 403/40303「店铺无操作权限」，不可见店铺 → 404「记录不存在或已被删除」；审单批量端点保持 200 信封、按行返回失败。本轮验证前端体验是否与该口径一致。
...
- `backend/internal/securitytests/permmatrix/r165_store_write_scope_test.go`：叠加合并解决——审单批量端点在 #332 语义下保持 200 信封、按行失败（「店铺无操作权限」）且不改 review_status，测试断言与之对齐。
