# R123 完整演示动线脚本（30 分钟 · Docker 全栈 + seed:demo:full，R128 增补自动化新动作，R132 增补 R128–R131 交付，R136 增补 R132–R135 交付，R141 增补 R136–R140 交付，R148 增补 R144–R147 交付，R153 增补 R148–R152 交付，R158 增补 R153–R157 交付，R163 增补 R158–R162 交付，R167 增补 view-only 演示点，R170 增补 R167–R169 口径，R175 增补 R170–R174 口径）
...
> R170 增补：R167–R169 交付不新增演示步骤，仅改口径（30 分钟总长不变）：① #332/#333 已合入 main，第 21b 步 view-only 写操作由后端拒绝并提示中文错误（403/40303）；前置禁用仅限审单工作台控件（R167 #335 收口面），订单列表/异常工作台等其余页面写控件仍可点击、点击后后端拒绝（R175 线2 实跑校正），审单批量含 view-only 店铺时整批 403/40303 零状态变更；② 第 23 步 `/settings/mcp-tokens` 页「配置方法见 docs/mcp.md 与 docs/open-api.md」已改为站内可点击链接（/docs/*.md，UX v10 P2-3 收口），可顺带点开展示；③ MCP 工具分页为钳制口径、开放 API 非法分页 400/40001，口径差异已写入 `docs/mcp.md`，口头带过即可。
> R175 增补：R170–R174 交付以收口/回归/决策材料为主，不新增演示步骤，仅改口径（30 分钟总长不变）：① 第 23 步 MCP 演示收尾时口头带过 **MCP 写白名单决策一页纸**已就绪待老板勾选（`docs/design/MCP_WRITE_WHITELIST_DECISION_BRIEF.md`，D1–D4 四项三视角辩论一致推荐采纳，#345 已合入 main；若老板当场问「下一步能不能让 AI 帮我操作」，即指向该页）；② 客服发送链路报错已中文化、seed `DEMO-SO-DELIVERED-0004` delivered_at 已修为过去时间（均随 #349，⏳ 待合并；基于 main 构建时该两点不可演示，不影响主动线）；③ 40303 文案「店铺无操作权限」已全站统一（#346/#347 已合入），第 21b 步 view-only 演示时可口头带过异常标记/迁移导入等面同口径且校验顺序先 scope 后 body；④ #348/#349/#350（客服复查/P2×4/大回归 v31）为报告/收口类交付不占演示时长，口径见验收包 §一/20；⑤ view-only 前置禁用口径收窄（见 R170 增补 ① 修正）：演示时以「点击后后端拒绝 + 中文 toast」为准，勿承诺全站前置禁用。
...
- 收尾零残留补充（R175 线2 实测）：`seed:demo:full:clean` 不覆盖 ① 非 `DEMO-` 前缀自建账号；② UI「删除用户」为软删除（`admin_users` 仍留 `deleted_at` 行）；③ 自建 `mcp_api_tokens` 行（即使已吊销）；④ `mcp_tool_call_logs` 行。verify 只查 `DEMO-` 行，上述残留存在时仍报 clean；需手工 psql 删除才能真正零残留。

## 实跑验证记录

- 2026-08-08（R175 线2）：Docker 全栈（`docker-compose.full.yml` 基于最新 main 构建，本轮 PR #351 为纯文档不影响镜像）三角色 + 临时 view-only 账号抽验实跑（录屏/截图证据外置不入库）：`DB_HOST=127.0.0.1 pnpm seed:demo:full` 成功；admin——dashboard KPI 有值、DEMO-AT-1004 批量标记已付款触发自动化（automation-logs 新增 success）、`/settings/mcp-tokens` 文档链接可点开、现场只读 token → `tools/list` 4 工具 → `orders_query`（pageSize=5）→ 吊销后 401/40101、`docs/design/MCP_WRITE_WHITELIST_DECISION_BRIEF.md` 在 main 且含决策一页纸；operator——11 vs 38 单、DEMO-AT-1005 批量标记已付款成功；readonly——写控件隐藏、读路径完整（提示：首次进入可能沿用上一账号 URL 的 keyword 过滤导致空列表，先点「重置」）；view-only（临时账号 scope=view）——数据可见 11 单、写操作（批量生成采购单、异常忽略）均后端拒绝 + 中文 toast「店铺无操作权限」零状态变化、异常 handle/ignore 空 body/无 body 均 403+40303（先 scope 后 body，#347 口径）；#349 未合并旧行为确认仍在（DEMO-SO-DELIVERED-0004 delivered_at 为未来时间戳，属预期）。18/19 项断言 PASS。**失实修正 ×1**：R170 增补「view-only 写控件已按店铺 scope 前置禁用」描述过宽——实测订单列表/异常工作台写控件仍可点击（仅状态不适用按钮禁用），拦截发生在提交后（403 中文 toast），前置禁用仅限审单工作台控件；已收窄 R170 增补措辞并在 R175 增补 ⑤ 登记。另登记收尾零残留补充（clean 不覆盖自建账号/软删除行/自建 MCP token/调用日志，见常见坑）。收尾 clean + verify 输出 `zero DEMO- residual rows`，临时账号/软删除行/自建 token/调用日志均手工清零。

- 2026-08-08（R170 线1）：Docker 全栈（`docker-compose.full.yml up -d --build` 全量重建，本轮 PR #342 分支：main + #339/#340 叠加 + R170 收口）实跑核对（录屏留证外置不入库）：`DB_HOST=127.0.0.1 pnpm seed:demo:full` 成功；admin——dashboard KPI（38 单）、`/settings/mcp-tokens` 文档链接可点击且新标签打开 `/docs/mcp.md`、`/docs/open-api.md`（200、text/plain 中文内联，mcp.md 含新「分页参数口径」小节）、现场创建只读 token 一次性明文、MCP `tools/list`→`orders_query`（pageSize=5 返回 5 条）、钳制口径 `{"page":0,"pageSize":999}` 不报错正常返回、开放 API `?page=0` → 400/40001、吊销后 MCP/开放 API 均 401（40101）；operator——仅授权店可见（11 vs 38 单）、DEMO-AT-1005 批量标记已付款成功；readonly——订单页写控件隐藏。12 项断言全部 PASS。**失实修正 ×1（本条即口径）**：MCP `orders_query` 响应数据键为 `items`（非 `list`），且不回显 page/pageSize，钳制值只能按返回行数间接验证。中途发现并即修 1 处缺陷：antd `Typography.Link` 生产构建丢 `target="_blank"`，改原生 `<a>` 后重建镜像复验通过（干净 Chromium runtime DOM 与 bundle 双证）。收尾 clean + verify 输出 `zero DEMO- residual rows`，现场 token 已吊销。
