# R123 完整演示动线脚本（30 分钟 · Docker 全栈 + seed:demo:full，R128 增补自动化新动作，R132 增补 R128–R131 交付，R136 增补 R132–R135 交付，R141 增补 R136–R140 交付，R148 增补 R144–R147 交付，R153 增补 R148–R152 交付，R158 增补 R153–R157 交付，R163 增补 R158–R162 交付）
...
> R163 增补：R158–R162 交付以安全/工程/文档面为主（#320/#321/#322/#323/#326 已合入 main，#324/#325/#327 ⏳ 待合并），不新增独立演示步骤，30 分钟时长不变：①大屏折算/自定义卡片（#318）已合入 main，基于最新 main 构建即可演示第 1b 步，不再需叠加分支；②报表币种设置未保存提示（R158 #320）可在第 19 步后顺带演示：`/settings/report-currency` 编辑汇率后底部出现「有未保存的更改，保存后才会生效」Tag，点「重新加载」还原后消失（约 20 秒，不影响总时长；路由跳转离开确认弹窗为 #327 ⏳，合入前不演示该点）；③MCP token 页审计卡已改名「MCP / 开放 API 调用审计日志」且工具筛选按「MCP 工具 / 开放 API」分组（#320），第 23 步预期已同步；④view-only 越权修复（#322）与 40303 业务码统一（#323）为安全面交付不占演示时长，可在第 21 步 readonly 演示时口头一句带过（店铺级 view 授权写操作后端 403 兑底）；⑤升级演练 R159（#321）、竞品复评 v8（#324 ⏳）、MCP 写白名单设计稿（#326，方案待决策）为报告/决策类交付不占演示时长，口径见验收包 §一/17；⑥UX v10 修复（#327 ⏳：备份/恢复页时间中文化、大屏趋势 tooltip 时间格式等）合入前第 23 步备份页创建时间列仍为 raw ISO、确认弹窗为英文 Cancel/OK，属已登记待合入项非故障。
...
- R163 时点 #327（UX v10 修复）尚未合入 main：备份/恢复页创建时间列为 raw ISO、创建备份确认弹窗英文 Cancel/OK、币种设置页路由离开无确认弹窗，均属已登记待合入项非故障，演示时不展示这些点或口头说明。
