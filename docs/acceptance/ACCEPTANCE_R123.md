...
- 日期：2026-08-05
- 基线：main `02b6b086`（#260 已合并）。**#261（R122 线1 性能收口 v2，perf/round122）截至本文撰写尚未合并**：性能收口为独立压测/慢查询治理轮，不影响本验收包描述的功能面；其证据在本文以「#261（待合并）」标注，合并后无需改动其余条目。
...
| 性能收口（万级列表/导出/并发压测、慢查询治理） | R116 导入万行 #249；R122 线1 #261（**待合并**） | #261 压测报告（合并后归档） | 🟡 #261 合并后即收口 |
...
1. #261（R122 性能收口 v2）待合并，合并后本表 §一/9「性能收口」条目状态由 🟡 转 ✅。
