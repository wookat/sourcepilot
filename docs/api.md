...
| `GET` | `/api/v1/inventory/logs` | 本地库存变更流水；行内附带 `productId` / `productSkuId` / `productTitle` / `skuCode` / `skuName` / `refOrderNo`（若有）便于溯源；支持 `warehouseId` 筛选，行内附 `warehouseId` / `warehouseName`（NULL 视为默认仓历史）。 |
| `GET` | `/api/v1/inventory/warehouses` | 仓库列表（R112，租户级）；返回 `{ list, total }`，含 `isDefault` / `enabled` / `priority`。 |
| `GET` | `/api/v1/inventory/warehouses/summary` | 各仓汇总（总库存 / 规格数）。 |
...
| `GET` | `/api/v1/orders/stats/daily/export.csv` | 导出经营报表逐日明细 CSV：`?days=30`（默认 30，最大 90），只读端点（readonly 可用），数据、口径与 scope 与 `stats/daily` 完全一致；UTF-8 BOM，列为「日期/订单数/已付款数/已发货数」+ 窗口内出现的每个币种两列「已付款销售额(币种)/折算金额(币种→本位币)」（币种字典序；无汇率时折算列显式输出「未折算」而非补 0）+「已付款销售额合计(本位币)/未折算币种」，空日期行补 0。 |
...
租户级自动化订单规则：状态事件触发（`order_created` 订单创建 / `order_paid` 已付款进入待采购 / `procurement_delivered` 采购签收入库 / `logistics_collected` 采购物流揽收）+ AND 条件（平台 / 店铺 / 金额区间 / 要求审单已通过）→ 一个站内自动动作（`confirm_payment` 自动确认付款（低风险：规则必须配置金额上限）/ `generate_procurement` 自动生成采购单 / `mark_printed` 自动标记打单 / `notify_shipping` 通知发货工作台 / `apply_shipping_rule` 自动应用发货规则 / `assign_warehouse` 自动分仓）。round126 新增动作：`apply_shipping_rule` 按 R111 发货规则推荐物流商并落到订单（`shippingApplyMode` ∈ `recommend` 仅推荐 / `apply` 直接应用，默认 `recommend`；仅记录计划物流商，发货时仍可人工改选，已有计划/人工选择不覆盖）；`assign_warehouse` 按策略（`warehouseStrategy` ∈ `default_warehouse` 默认仓 / `stock_first` 库存充足优先，默认 `default_warehouse`）为待发货订单分配发货仓，分配结果与多仓扣减联动（未显式指定仓时扣减锁定已分配仓）；库存不足时执行失败留痕（可在执行日志重试）。两动作的落库结果在订单详情 `GET /api/v1/orders/:id` 返回体中透出：`plannedCarrierCode/Name/Mode/Rule/At` 与 `assignedWarehouseId/Name/Strategy`、`warehouseAssignedAt`（均可选字段，订单概览展示「计划物流商 / 分配仓库」）。失败日志文案口径：`执行失败（本轮尝试 N 次）：...` 表示本轮 inline 尝试次数，执行日志的「尝试次数」列为历次累计。规则按 `priority` 升序执行，命中的每条规则都会执行；动作按 `租户:规则:订单:事件` 幂等（成功/跳过不重复执行），失败 inline 最多 3 次尝试并可在执行日志人工重试。安全边界：`pending_review` / `held` 订单一律不自动化（沿用审单阻断），审单放行后已付款订单补触发 `order_paid`。自动动作在操作日志留痕，消息含「自动规则：规则名」。仅站内状态流转，不调用真实平台 API。写端点要求非 readonly；全部按租户隔离，越权返回 404。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `GET` | `/api/v1/order-automation-rules` | 规则列表（租户隔离，按 `priority` 升序）：返回 `{items:[{id, name, priority, enabled, triggerEvent, action, minAmount?, maxAmount?, platforms?, shopIds?, requireReviewPassed, shippingApplyMode?, warehouseStrategy?}]}`。 |
| `POST` | `/api/v1/order-automation-rules` | 新增规则：`{name, priority?, enabled?, triggerEvent, action, minAmount?, maxAmount?, platforms?, shopIds?, requireReviewPassed?, shippingApplyMode?, warehouseStrategy?}`；`triggerEvent`/`action` 必须为合法组合；金额区间非负且 min≤max；`confirm_payment` 必须设置 `maxAmount`；`apply_shipping_rule` 默认 `shippingApplyMode=recommend`，`assign_warehouse` 默认 `warehouseStrategy=default_warehouse`，非法取值 400。 |
...
| `GET` | `/api/v1/reports/profit/export.csv` | 利润报表 CSV 导出（参数同上）：UTF-8 BOM，含原币列与折算列（无汇率时折算列与本位币列显式输出「未折算」而非补 0）、成本/费用/毛利/毛利率/缺进价行数/未折算币种，数据与 `reports/profit` 完全一致。 |
...
| `GET` | `/api/v1/finance/reconciliation/export.csv` | 差异工作台 CSV 导出（UTF-8 BOM，csvsafe；参数同上；无汇率时本位币列显式输出「未折算」而非留空）。 |
...
| `GET` | `/api/v1/finance/report/export.csv` | 对账报表 CSV 导出（UTF-8 BOM，csvsafe；参数同上；无汇率时本位币列显式输出「未折算」而非留空，店铺月度费用无登记时仍留空）。 |
...
| `GET` | `/api/v1/imports/templates/:kind?format=csv|xlsx` | 通用导入模板下载：默认 UTF-8 BOM CSV（表头为目标字段中文标签 + 一行示例）；`format=xlsx` 返回真实 xlsx 工作簿（R116）。 |
| `GET` | `/api/v1/imports/progress?kind=&fileHash=` | 当前批次导入进度（R116）：`active / processed / total`；未在导入时 `active=false`。导入进行中重复提交 commit 返回 409（该批次正在导入中）。进度为单实例内存态，仅作前端体验提示，不作为业务状态依据。 |
...
GET  /api/v1/mcp/audit-logs         # 工具调用审计日志（租户 scope），query: page/pageSize/tool/status/mode；行含 amount（仅金额型写动作 procurement_mark_paid 有意义，其余为 0/缺省）；写动作审计行（有 mode 的行与写白名单工具的行）仅 settings.manage 持有者可见，其余角色自动过滤（R184）
```
...
- 审计日志（R146）：每次 MCP `tools/call` 落一条（tenant / token 脱敏标识 / 工具名 / 时间 / 成败 / 耗时），不记录查询参数与结果内容；四角色可读（R184 起写动作审计行仅 settings.manage 持有者可见）。
