...
## round177 settings 敏感 key 服务端注册表（R176 P2① 收口）

- **服务端声明式敏感 key 注册表**：`settings.IsSensitiveKey(groupKey, itemKey)` 由服务端注册表决定敏感配置项（AI key、平台凭证、S3/COS/OSS 秘钥、SMTP 密码、webhook secret 等）必须加密落库与脱敏回显，不再信任客户端 `isEncrypted` 声明（新建项路径含内）。注册表来源：① `settings.IntegrationConfigDefinitions()` 中 `Sensitive: true` 字段（ai / image / storage / mail / alert_notify）＋ legacy `email/smtp_password`（包 init 静态注册）；② 各平台 `AppConfigSchema` / `PublishConfigSchema` 的 `Sensitive` 字段（`api.Register` 内 provider 全部注册后经 `settings.RegisterSensitiveKeys` 汇入，覆盖 `platform_*` 全组）。非注册表项保持客户端声明兼容（显式 `isEncrypted:true` 仍可加密）。
- **实现**：`settings.putOne` 加密判定为 `it.IsEncrypted || (exists && cur.IsEncrypted) || IsSensitiveKey(gk, ik)`，与 R176 加密粘性叠加；密文/掩码/`PlainByGroup` 解密回读三条链路口径不变。
- **新增回归测试**（均先红后绿）：`settings.TestPutBulkNewSensitiveKeyForcedEncrypted`（新建项显式 `isEncrypted:false` 仍加密落库+List 脱敏+PlainByGroup 回读）、`settings.TestPutBulkNonRegistryKeyKeepsClientBehaviour`（非注册表项兼容）、`settings.TestSensitiveRegistryCoversPlatformCredentialShape`；`permmatrix.TestSettingsSensitiveRegistryForcesEncryptionOnCreate`（生产路由 `PUT /api/v1/settings` 四组代表 key：`ai/qwen_api_key`、`storage/s3_secret_access_key`、`platform_tiktok/app_secret`、`alert_notify/webhook_secret`，断言密文落库 + `GET /settings` 脱敏）。permmatrix harness 补 `Encrypter` 注入（此前为 nil，敏感项保存会 400）。
