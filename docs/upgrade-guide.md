...
- 2026-08-07（R159 季度复跑）：旧版本（R149 时点 main `7f5645c1`）+ 存量数据（双业务租户 2 万订单/4 万订单行/6 万库存流水/2 万自动化执行日志/4000 SKU/4000 回款/存量 MCP token/存量话术模板/跨租户重复订单号样本）→ 新版本（main `32a9aaea`，含 #308/#309/#313/#315/#316/#320 与升级中途合入的 #312 批次）全流程演练通过：从零部署（production + Caddy 内部 CA）空库到登录实测 165s、含镜像构建的升级部署 464s（#312 复跑 246s，均 <15 分钟目标）；`--pre-upgrade-check` 备份 + 预检 0 行（`BACKUP_DIR` 不可创建/不可写时现版本分别报「无法创建备份目录」与裸 `pg_dump 备份失败`，后者待 #317 -w 检查收口）；升级后 R152 `mcp_api_tokens.purpose` 列（存量 5 行回填 `mcp`）、`customer_reply_template_variants` 新表、`buyer_message_drafts.language/lang_source` 列全部落地，`order_automation_logs.shop_id` 回填补齐存量 1 万 NULL 行且与 orders 逐行 0 偏差；订单金额/订单行/库存流水/SKU 库存/回款/token/模板指纹逐项 0 差异；升级后实测：存量 `purpose=mcp` token MCP 可用、调开放 API 401，新建 `openapi`/`both` token 各口径正确（`openapi` 调 MCP 401、`both` 两侧可用）、开放 API 限流（RPS5/burst10 第 11 个起 429）、租户隔离（tenant-b token 仅见本租户订单）、审计落 `openapi:*` 行、R154 非法 `pageSize=0` 返回 400、多语言模板变体（en/es 创建/落表/读取）、`OPENAPI_ENABLED=false` 入口 404；`TRUSTED_PROXIES`（#311/#312 + #316 配置面）实测：配置 compose 网段时 client_ip 为真实客户端且外部伪造 XFF 无效，留空时 client_ip 退化为 admin 容器 TCP peer（共享预算口径与文档一致）；`stop backend` → `docker exec -i` + `pg_restore --clean --if-exists` 恢复升级前备份（指纹与升级前逐项一致，`customer_reply_template_variants` 新表残留符合既有口径）→ 重启迁移幂等重跑（指纹与升级后一致、purpose 列/回填重新落地）。大屏折算/自定义指标与汇率 seed（#318）、`--pre-upgrade-check` -w 新报错口径（#317）本轮未合入 main，不在迁移面，待合入后下轮补验。
