...
| MCP 只读入口（round144） | `backend/internal/modules/mcptoken`（model/service/handler/router）、`backend/internal/modules/mcpserver`（server/tools/context）、`backend/internal/api/router.go`（`POST /api/mcp` + `/api/v1/mcp/tokens`）、`backend/internal/database/migrate.go`、`backend/internal/config/config.go`（`MCP_ENABLED`/`MCP_RATE_RPS`/`MCP_RATE_BURST`）、`backend/internal/securitytests/permmatrix/matrix.json`、`tests/contracts/api-contracts.json`、`admin/src/services/mcpTokens.ts`、`admin/src/pages/Settings/McpTokens.tsx`、`admin/config/routes.ts`、`admin/e2e/specs/round144-mcp-tokens.spec.ts`、`.env.example`、`.env.docker.example`、`docs/mcp.md`、`docs/env.md`、`docs/api.md`、`docs/permission-matrix.md`、`SECURITY.md` |
| 开放 API 只读入口（round152） | `backend/internal/modules/openapi`（server/handler）、`backend/internal/modules/readonlyquery`（MCP/开放 API 共享只读查询）、`backend/internal/modules/mcptoken`（`purpose` 字段：mcp/openapi/both）、`backend/internal/modules/mcpserver/tools.go`（改走 readonlyquery）、`backend/internal/api/router.go`（`GET /api/open/v1/*`）、`backend/internal/config/config.go`（`OPENAPI_ENABLED`/`OPENAPI_RATE_RPS`/`OPENAPI_RATE_BURST`）、`backend/internal/securitytests/permmatrix/matrix.json`、`tests/contracts/api-contracts.json`、`docs/openapi/open-api.v1.json`（OpenAPI 3 规范，`spec_test.go` 契约校验）、`admin/src/services/mcpTokens.ts`、`admin/src/pages/Settings/McpTokens.tsx`（用途选择/用途列）、demo seed（`fulldemo_round147.go` 开放 API 演示 token）、`.env.example`、`.env.docker.example`、`docs/open-api.md`、`docs/env.md`、`docs/api.md`、`docs/mcp.md`、`docs/permission-matrix.md` |
| 买家消息多语言模板（round152） | `backend/internal/modules/customerchat`（template/template_lang/template_service、buyermsg_model/buyermsg_generate/buyermsg_drafts/buyermsg_handler/router）、`backend/internal/database/migrate.go`（`customer_reply_template_variants`）、`backend/internal/modules/demoseed`（`fulldemo_selection_cs.go`、`fulldemo_round119_buyermsg.go`、clean/verify）、`tests/contracts/api-contracts.json`、`admin/src/services/customer.ts`（`TEMPLATE_LANGUAGES`）、`admin/src/pages/Customer/ReplyTemplates`、`admin/src/pages/Customer/BuyerMessages`、`admin/e2e/specs/round152-multilang-templates.spec.ts`、`docs/api.md`、`docs/DEMO_SEEDING_GUIDE.md` |
