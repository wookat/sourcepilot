...
func (s *Service) GetTodos(ctx context.Context, q Query, sc Scope) (*TodosDTO, error) {
	...
	items := []UnifiedTodo{
		...
		unifiedTodo("procurement_pending_confirm", "P1", "采购单待确认", "采购单已核价，等待人工确认", "procurement", "", "/procurement?status=pending_confirm", sum.ProcurementPendingConfirmCount),
		unifiedTodo("procurement_placing", "P0", "待去 1688 下单", "已确认采购单等待到 1688 下单并回填订单号", "procurement", "", "/procurement?status=placing", sum.ProcurementPlacingCount),
		unifiedTodo("procurement_unpaid", "P0", "采购单待付款", "已下单未付款，请到 1688 完成付款并标记", "procurement", "", "/procurement?status=placed", sum.ProcurementUnpaidCount),
		unifiedTodo("procurement_await_tracking", "P1", "待回填运单号", "已付款采购单等待回填快递单号（支持批量粘贴）", "procurement", "", "/procurement?status=paid", sum.ProcurementAwaitTrackingCount),
