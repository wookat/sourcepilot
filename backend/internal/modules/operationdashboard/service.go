...
func buildTodoCards(sum *Summary, publishable int64) []TodoCard {
	return []TodoCard{
		...
		todoCard("order_sku_unmatched", "订单 SKU 未匹配", sum.SKUUnmatchedOrderItems, failureclassifier.SeverityHigh,
			"订单行未匹配到本地 SKU，需人工匹配", "/orders/exceptions?exceptionType=sku_unmatched"),
