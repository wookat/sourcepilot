...
// Handler exposes order sync routes.
type Handler struct {
	Svc *Service
}
...
// SyncShopOrders POST /shops/:id/sync-orders
func (h *Handler) SyncShopOrders(c *gin.Context) {
	if h == nil || h.Svc == nil {
	...
	if err != nil {
		...
		case errors.Is(err, platformp.ErrManualOrderSyncUnsupported):
			...
			return
		case errors.Is(err, platformp.ErrOrderSyncNotImplemented):
...
// ListTasks GET /order-sync/tasks
...
// GetTask GET /order-sync/tasks/:id
...
// RetryTask POST /order-sync/tasks/:id/retry
