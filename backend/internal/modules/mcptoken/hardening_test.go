...
func TestAuthenticateRejectsNonReadonlyScope(t *testing.T) {
	...
	res, err := svc.Create(context.Background(), 1, "scope-check", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateEnforcesActiveTokenCap(t *testing.T) {
	...
	for i := 0; i < mcptoken.MaxActiveTokensPerTenant; i++ {
		res, err := svc.Create(context.Background(), 1, fmt.Sprintf("tok-%d", i), nil)
		if err != nil {
			t.Fatalf("create %d: %v", i, err)
		}
		lastID = res.Token.ID.String()
	}
	if _, err := svc.Create(context.Background(), 1, "over-cap", nil); !errors.Is(err, mcptoken.ErrTooManyTokens) {
		t.Fatalf("over cap: err = %v, want ErrTooManyTokens", err)
	}
	// Another tenant is unaffected by this tenant's cap.
	if _, err := svc.Create(context.Background(), 2, "other-tenant", nil); err != nil {
	...
	if _, err := svc.Create(context.Background(), 1, "after-revoke", nil); err != nil {
